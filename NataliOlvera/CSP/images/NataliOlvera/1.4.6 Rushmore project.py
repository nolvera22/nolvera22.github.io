...
def ovalmask(image): #ovalmask is applied to first image
    ...
    length = image.size[1] #length is equal to the image size on the second value (y)
    my_mask = PIL.Image.new('RGBA', (width, length), (0, 0, 0, 0))#the length and width for my_mask
    drawing_layer = PIL.ImageDraw.Draw(my_mask) #
    drawing_layer.ellipse((690, 72, 920, 388), fill=(255, 0, 0, 255))#changes my_mask for images, fill is for backrgound
    result = PIL.Image.ellipse((175, 300, 520, 890), fill=(255, 0, 0, 255))#changes my_mask for images, fill is for backrgoune.new('RGBA', (width, length))
    result.paste(image, (0, 0), mask=my_mask)#shows new image as result to what you change
    return result
